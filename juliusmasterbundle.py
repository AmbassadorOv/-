...
"""
Usage:
- Install dependencies: pip install flask apscheduler requests
- Run: python juliusmasterbundle.py
- Operator must run deploy actions manually; this service provides endpoints and utilities.
"""
...
# Bridge Integration
# -------------------------
@app.route("/bridge/heartbeat", methods=["GET"])
def bridge_heartbeat():
    from moon_wheel_bridge import bridge
    return jsonify(bridge.heartbeat())

@app.route("/bridge/commands", methods=["GET"])
def bridge_commands():
    from moon_wheel_bridge import bridge
    return jsonify({"queue_size": len(bridge.command_queue), "status": bridge.last_sync_status})

@app.route("/bridge/pop_command", methods=["POST"])
def bridge_pop_command():
    from moon_wheel_bridge import bridge
    if bridge.command_queue:
        command = bridge.command_queue.pop(0)
        return jsonify(command)
    return jsonify({"status": "empty"}), 204

@app.route("/bridge/report", methods=["POST"])
def bridge_report():
    from moon_wheel_bridge import bridge
    payload = request.get_json(force=True)
    report_json = json.dumps(payload)
    response = bridge.receive_from_jule(report_json)
    return jsonify({"bridge_response": response})
